...
class DifferentialKinematics():
    def __init__(self, wheel_radius, wheel_separation):
        self.right_wheel_Rotational_Speed = rospy.Publisher('/wheel_right_controller/command', Float64, queue_size=10)
        self.left_wheel_Rotational_Speed = rospy.Publisher('/wheel_left_controller/command', Float64, queue_size=10)
        ...
        self.dynamic_msg.child_frame_id = "base_footprint"
        ...
        self.Conversion_From_Real_World_to_Robot_Axis_Matrix = np.array([
            [wheel_radius / 2, wheel_radius / 2],
            [wheel_radius / wheel_separation, -wheel_radius / wheel_separation]
        ])
        self.listen = rospy.Subscriber("/Real_World_cmd_vel", Twist, self.Callback)
        self.State_listener = rospy.Subscriber("/joint_states", JointState, self.inverKinemetic)
        self.t0 = rospy.Time.now()

    def Callback(self, msg):
        Real_World_speed = np.array([[msg.linear.x], [msg.angular.z]]) #idk know he not getting speed in y #he is only giving speed in x 
        RotationSpeed = np.matmul(np.linalg.inv(self.Conversion_From_Real_World_to_Robot_Axis_Matrix), Real_World_speed)
        Right_wheel_Rotate = Float64(RotationSpeed[0, 0])
        Left_wheel_Rotate = Float64(RotationSpeed[1, 0])
        self.right_wheel_Rotational_Speed.publish(Right_wheel_Rotate)
        self.left_wheel_Rotational_Speed.publish(Left_wheel_Rotate)

    def inverKinemetic(self, msg):
        # Calculate change in position since the previous iteration
        dp_left = msg.position[0] - self.left_wheel_prev_position
        dp_right = msg.position[1] - self.right_wheel_prev_position
...
if __name__ == "__main__":
    ...
    kinematics = DifferentialKinematics(wheel_radius, wheel_separation)
    rospy.spin()
